...
function insertarNodoP(proceso,nombre, tiempo, quantum,tll, tf, tr,  pr,rar){
	var nuevo = new nodo();
...
function insertarNodoU(proceso,nombre, tiempo, quantum,tll, tf, tr,  pr,rar){
	var nuevo = new nodo();
	...
	if(this.vacia()){
		...
		while(!this.vacia()){	
			...
			colaTemp.insertarPrimero(temp.proceso,temp.nombre, temp.tiempo, temp.quantum,temp.Tllegada,
				temp.Tfinalizacion,temp.Turnarround,temp.prioridad,temp.rafagareal); 		
		}
		this.insertarPrimero(proceso,nombre, tiempo, quantum,tll, tf, tr,  pr,rar);		
